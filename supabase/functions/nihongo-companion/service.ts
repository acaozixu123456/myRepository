...
export class ServiceError extends Error {constructor(public reason:string,public status=503){super(reason);}}
...
export async function provider(key:string,path:string,init:RequestInit,timeout=23000){
  ...
  if(!r.ok){const body=await r.json().catch(()=>({}));const code=body?.error?.code;
    throw new ServiceError(code==='insufficient_quota'?'provider_credit':r.status===429?'provider_busy':r.status===404?'model_unavailable':'provider_request_failed',r.status===429?429:502);
  }return r;
}
...
export async function jsonModel(key:string,request:Record<string,unknown>){const r=await provider(key,'/responses',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({store:false,...request})});const data=await r.json();if(data.status!=='completed')throw new ServiceError('generation_incomplete',502);const text=(data.output||[]).flatMap((o:any)=>o.content||[]).filter((c:any)=>c.type==='output_text').map((c:any)=>c.text||'').join('');return {data,text};}
