...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const {signature,...unsigned}=proposed;const local=LOCAL_SEEDS.find(s=>s.id===proposed.id);
    const seed=local||proposed;
  ...
  }catch(e){return reply({ok:false,reason:e instanceof ServiceError?e.reason:'service_unavailable'},e instanceof ServiceError?e.status:503);}
});
