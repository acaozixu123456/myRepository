...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(['heartbeat','stop','observe','feedback','lesson','demo'].includes(action)){
   ...
   if(action==='lesson')return reply({ok:true,...await teacherLesson(k,body)});
   if(action==='demo')return reply({ok:true,...await teacherDemo(k,body)});
