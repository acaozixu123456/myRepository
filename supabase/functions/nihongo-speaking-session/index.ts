...
async function guard(key:string,id:string,expiry:number){
  ...
  ws.on('message',data=>{try{const e=JSON.parse(data.toString());if(e.type==='response.created'&&++replies>12)void stop('response_limit');if(e.type==='response.done'){tokens+=Math.max(0,Number(e.response?.usage?.total_tokens)||0);if(tokens>30000)void stop('token_limit');}if(e.type==='session.updated'){const s=e.session||{};if((s.model&&s.model!==MODEL)||Number(s.max_output_tokens)>384||s.audio?.input?.turn_detection?.create_response===true)void stop('session_policy_changed');}}catch{void stop('invalid_guard_event');}});
...
Deno.serve(async(request:Request)=>{
  ...
  try{
    if(body.action==='health'){
      ...
      return json({ok:probe.ok,model:MODEL,modelReady:probe.ok,contractVersion:SPEAKING_CONTRACT,audioPersistedByApp:false,maxSeconds:SPEAKING_SECONDS,maxResponses:12,globalStartsPerDay:30,individualAccountAuth:false},probe.ok?200:503);
    }
    if(body.action==='stop'){
    ...
    if(!await quota('speaking-global',30,1440)||!await quota(`speaking-client:${body.clientKey}`,8,60)||!await quota(`speaking-concurrent:${body.clientKey}`,2,2))return json({ok:false,reason:'speaking_quota'},429);
    const session={type:'realtime',model:MODEL,instructions:speakingInstructions(plan,0,'start'),output_modalities:['audio'],max_output_tokens:384,tools:[],tool_choice:'none',audio:{input:{noise_reduction:{type:'near_field'},transcription:{model:'gpt-4o-mini-transcribe',prompt:'The speaker is learning Japanese and may ask for help in Chinese. Transcribe only speech actually heard; never invent words from a lesson.'},turn_detection:{type:'semantic_vad',eagerness:'low',create_response:false,interrupt_response:false}},output:{voice:'marin',speed:0.93}}};
    ...
    if(!upstream.ok){const e=await upstream.json().catch(()=>({}));const parameter=typeof e.error?.param==='string'&&/^[a-zA-Z0-9._\[\]]{1,90}$/.test(e.error.param)?e.error.param:undefined;return json({ok:false,reason:upstream.status===404?'model_unavailable':upstream.status===429?'provider_quota':'realtime_config_error',upstreamStatus:upstream.status,parameter},502);}
    ...
    return json({ok:true,contractVersion:SPEAKING_CONTRACT,model:MODEL,sdp,callId,expiresAt,stopToken:await signature(callId,expiresAt),audioPersistedByApp:false});
