...
export async function companionProxy(req:VercelRequest,res:VercelResponse,body:Record<string,unknown>,config:{url:string;anonKey:string;clientKey:string}){
  ...
  const action=String(body.action||'').replace(/^companion_/,'');if(!['health','start','heartbeat','stop','topics','observe'].includes(action))return res.status(400).json({ok:false,reason:'invalid_action'});
