...
/** Keep Node's module graph independent of the Deno/client chat module.
 * Source/size validation stays here; the edge owns canonical topic validation. */
export function proxySpeakingPlan(raw:unknown){
  const base=validateSpeakingPlan(raw);
...
export async function handleSpeakingProxy(req:VercelRequest,res:VercelResponse,body:Record<string,unknown>,config:Config){
  ...
  const action=String(body.action||'');let payload:Record<string,unknown>;
