...
replace('const result=await call(body);if(!result.data.ok',"const result=await call(body);if(body.action==='companion_feedback'){report.feedbackRequests??=[];report.feedbackRequests.push({mode:body.input?.mode,status:result.status,reason:result.data.reason,disposition:result.data.disposition,model:result.data.model,note:result.data.note||null});}if(!result.data.ok");
