...
try{
 for(const engine of ['chromium','webkit'])for(const viewport of [{width:390,height:844},{width:1280,height:900}]){
  ...
  await page.getByRole('button',{name:'场景',exact:true}).click();await page.locator('.imm-scene-panel').waitFor();
  await page.locator('.imm-scene-panel button').filter({hasText:'只看风景'}).click();
  const exit=page.locator('button').filter({hasText:/返回.*(?:练|聊)/}).filter({visible:true}).first();await exit.waitFor();
...
}catch(e){reports.push({ok:false,failure:e.stack||String(e)});process.exitCode=1;}finally{await browser?.close();await writeFile(out+'/result.json',JSON.stringify(reports,null,2));console.log('ACTUAL_SCENERY_CHECK',JSON.stringify(reports));}
