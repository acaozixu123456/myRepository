...
const workerPath = String(config.workerPath || join(dirname(configPath), 'windows-cursor-git-task-v2.mjs'));
...
function publish(message) {
  ...
  if (runGit(['diff', '--cached', '--quiet'], false).status === 0) return;
...
function executorSnapshot() {
  const healthy = Boolean(cursorCli && existsSync(cursorCli));
  return { healthy, version: healthy ? 'present-not-probed' : `missing:${cursorCli}` };
}
...
function healthPayload() {
  const ex = executorSnapshot();
  return {
    agent_id: agentId,
    version: VERSION,
    transport: 'git-polling',
    provider: 'cursor',
    executor_healthy: ex.healthy,
    executor_version: ex.version,
...
function execute(command) {
  ...
  if (method === 'health') return healthPayload();
...
function loadSeen() {
  ...
  try { return new Set(readJson(seenFile)); } catch { return new Set(); }
}
...
function writeAgentDescriptor() {
  writeJsonAtomic(join(agentsDir, `${safeId(agentId)}.json`), healthPayload());
}
...
while (true) {
  try {
    ...
    for (const name of files) {
      ...
      if (existsSync(resultPath)) { seen.add(name); continue; }
      let command;
      try { command = readJson(commandPath); }
        ...
        seen.add(name); changed = true; continue;
      }
      if (command.agent_id && String(command.agent_id) !== agentId) { seen.add(name); continue; }
