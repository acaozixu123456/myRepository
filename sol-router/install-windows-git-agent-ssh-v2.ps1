...
function New-RepositorySshKey([string]$SshKeygen,[string]$PrivateKeyPath,[string]$PublicKeyPath,[string]$Comment){
  ...
  if($proc.ExitCode -ne 0 -or -not(Test-Path $PrivateKeyPath) -or -not(Test-Path $PublicKeyPath)){
    throw ('SSH key generation failed exit={0}' -f $proc.ExitCode)
  }
}
...
Step 'Generating repository-only SSH key (non-interactive)'
...
Write-Host '  1. The Deploy Keys page is opening.' -ForegroundColor Yellow
Write-Host '  2. Click "Add deploy key".' -ForegroundColor Yellow
Write-Host ('  3. Title: {0}' -f $AgentId) -ForegroundColor Yellow
Write-Host '  4. Paste the key (already copied).' -ForegroundColor Yellow
Write-Host '  5. Check "Allow write access" and save.' -ForegroundColor Yellow
Write-Host ''
...
try {
  Step 'Waiting for the deploy key to become active (Ctrl+C cancels)'
  $attempt=0
  while($true){
    $attempt++
  ...
  if($pushCheck -ne 0){
    Start-Process $DeployKeyPage
    throw 'Deploy key is readable but does not have write access; enable Allow write access'
  }
} finally {
