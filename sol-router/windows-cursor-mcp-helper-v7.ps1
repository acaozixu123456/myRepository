...
function Write-Envelope($Value){
  [IO.File]::WriteAllText($ResultFile,($Value|ConvertTo-Json -Depth 60 -Compress))
}
function Find-Node {
...
function Start-Mcp {
  ...
  $init=@{protocolVersion='2025-03-26';capabilities=@{};clientInfo=@{name='sol-router-windows-helper';version='0.7.0'}}
...
function ParseTool($r){
  ...
  $t=(ToolText $r).Trim();if($t){try{return $t|ConvertFrom-Json}catch{return @{text=$t}}}
  return $r
}
...
function ToolProps([string]$Name){
  $tool=$script:Tools[$Name];$h=@{};if(-not $tool){return $h}
...
function WorkspaceNames($v){
  $raw=if($v.workspaces){@($v.workspaces)}elseif($v.items){@($v.items)}else{@($v)};$out=@()
  foreach($item in $raw){
...
function MapStart($p){
  $props=ToolProps 'cursor_start';$a=@{};$w=[string]$p.workspace;$prompt=[string]$(if($p.prompt){$p.prompt}elseif($p.task){$p.task}else{''});$model=[string]$p.model
...
function MapStatus($p){
  $props=ToolProps 'cursor_status';$rid=[string]$(if($p.runId){$p.runId}elseif($p.run_id){$p.run_id}else{$p.id});if(-not $rid){throw 'run_id_required'}
...
try{
  ...
  switch($Method){
    ...
    'start'{
      $w=[string]$payload.workspace;$available=@(WorkspaceNames (CallTool 'cursor_list_workspaces' @{}))
