...
function Run-Git([string[]]$GitArgs,[switch]$AllowFailure){
  ...
  if(-not $AllowFailure -and $code -ne 0){throw ('git failed exit={0}: git {1}' -f $code,($GitArgs -join ' '))}
  return $code
}
...
$probe=Run-Git @('ls-remote','--exit-code',$RepoUrl,"refs/heads/$Branch") -AllowFailure
if($probe -ne 0){throw 'Repository URL cannot read the relay branch'}

Step 'Cloning relay'
if(Test-Path $RelayDir){Remove-Item -Recurse -Force $RelayDir}
Run-Git @('clone','--branch',$Branch,'--single-branch',$RepoUrl,$RelayDir) | Out-Null
Run-Git @('-C',$RelayDir,'config','user.name','sol-router-git-agent') | Out-Null
Run-Git @('-C',$RelayDir,'config','user.email','sol-router-git-agent@users.noreply.github.com') | Out-Null
Run-Git @('-C',$RelayDir,'remote','set-url','origin',$RepoUrl) | Out-Null
$pushProbe=Run-Git @('-C',$RelayDir,'push','--dry-run','origin',"HEAD:$Branch") -AllowFailure
...
Write-Host '  transport: Git polling over verified HTTPS repository URL'
