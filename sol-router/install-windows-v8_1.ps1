...
$StageAgent=Join-Path $InstallRoot 'windows-cursor-agent-v8.stage.ps1'
$StageWorker=Join-Path $InstallRoot 'windows-cursor-task-v8.stage.mjs'
$WorkspaceFile=Join-Path $InstallRoot 'workspaces.json'
$WorkspaceStage=Join-Path $InstallRoot 'workspaces.stage.json'
$Runner=Join-Path $InstallRoot 'run-agent.ps1'
$RunnerStage=Join-Path $InstallRoot 'run-agent.stage.ps1'
...
foreach($stale in @($StageAgent,$StageWorker,$WorkspaceStage,$RunnerStage)){
  Remove-Item -Force $stale -ErrorAction SilentlyContinue
}

Step 'Downloading v0.8.1 installer payload to extension-safe staging files'
...
Step 'v0.8.1 installer finished'
