...
const runId = String(spec.runId || 'run');
...
// Cursor CLI has had argv parsing bugs with embedded -p/-P in positional prompts.
// Keep the positional prompt fixed and ASCII-safe; put the real task in a local file.
const safeRun = runId.replace(/[^A-Za-z0-9]/g, '_');
const promptDir = join(dirname(stateFile), 'prompts');
mkdirSync(promptDir, { recursive: true });
const promptFile = join(promptDir, `${safeRun}.txt`);
writeFileSync(promptFile, prompt, 'utf8');
const cliPrompt = `Read the task instructions from ${promptFile} and execute them exactly in the current workspace.`;

const cursorArgs = ['--print', '--output-format', 'stream-json'];
if (spec.model) cursorArgs.push('--model', String(spec.model));
cursorArgs.push(cliPrompt);
...
patchState({ state: 'running', phase: 'launching', cursorCli: cli, workerPid: process.pid, startedAt: now(), promptFile, lastOutput: '' });
...
function handleLine(line) {
  ...
  patchState({ state: 'running', phase: phaseOf(obj), lastOutput: textOf(obj, line), lastEvent });
}
