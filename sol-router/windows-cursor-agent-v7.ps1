...
function Check-LocalPrereqs{
  $stdio=Join-Path $RouterRoot 'mcp\dist\src\stdio.js';$config=Join-Path $RouterRoot 'mcp\config\config.json'
...
function Ws-SendText($Ws,[string]$Text){$b=[Text.Encoding]::UTF8.GetBytes($Text);$s=[ArraySegment[byte]]::new($b);$Ws.SendAsync($s,[Net.WebSockets.WebSocketMessageType]::Text,$true,[Threading.CancellationToken]::None).GetAwaiter().GetResult()}
...
function Invoke-Helper([string]$Method,$Payload,[int]$TimeoutMs){
  $id=[Guid]::NewGuid().ToString('N');$payloadFile=Join-Path $env:TEMP "sol-router-$id-in.json";$resultFile=Join-Path $env:TEMP "sol-router-$id-out.json"
  try{
...
function Start-Identity($Payload){
  $prompt=[string]$(if($Payload.prompt){$Payload.prompt}elseif($Payload.task){$Payload.task}else{''});$key=''
  if($prompt -match '^\[\[SOL_ROUTER_COMMAND_ID:([A-Za-z0-9._:-]{1,160})\]\](?:\r?\n)?'){$key=[string]$Matches[1];$prompt=$prompt.Substring($Matches[0].Length)}
  $copy=@{};foreach($p in $Payload.PSObject.Properties){$copy[$p.Name]=$p.Value};$copy['prompt']=$prompt;$copy.Remove('task')|Out-Null
  return @{key=$key;payload=$copy}
}
function JournalPath([string]$Key){$safe=$Key -replace '[^A-Za-z0-9._-]','_';return Join-Path $JournalDir "$safe.json"}
function Execute-Command([string]$Method,$Payload){
  switch($Method){
    ...
    'list_workspaces'{
      $r=Invoke-Helper 'list_workspaces' @{} 30000;$script:KnownWorkspaces=@($r.workspaces);return $r
    }
    'start'{
      $identity=Start-Identity $Payload
      if($identity.key){$jp=JournalPath $identity.key;if(Test-Path $jp){return(Get-Content $jp -Raw|ConvertFrom-Json)}}
...
while($true){
  ...
  try{
    ...
    while($ws.State -eq [Net.WebSockets.WebSocketState]::Open){
      $text=Ws-ReceiveText $ws;if($text -eq 'pong'){continue};try{$msg=$text|ConvertFrom-Json}catch{continue};if($msg.type -ne 'command'){continue}
      $cid=[string]$msg.id;$method=[string]$msg.method;$payload=if($msg.payload){$msg.payload}else{@{}};Log "command=$method id=$cid"
      try{$result=Execute-Command $method $payload;Ws-SendJson $ws @{type='result';id=$cid;ok=$true;result=$result};Log "result=$method ok";if($method -eq 'restart'){Start-Sleep -Milliseconds 200;exit 0}}
