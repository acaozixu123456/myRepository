param(
  [string]$RepoSsh='git@github.com:acaozixu123456/sol-luna-accelerator.git',
...
function Find-SshKeygen([string]$GitExe){
  ...
  foreach($candidate in @(
    (Join-Path $gitRoot 'usr\bin\ssh-keygen.exe'),
...
if(-not $sshKeygen){throw 'ssh-keygen.exe not found (Windows OpenSSH or Git for Windows SSH tools are required)'}
...
if(-not(Test-Path $KeyPath)){
  Step 'Generating repository-only SSH key'
  $code=Invoke-Native $sshKeygen @('-t','ed25519','-N','','-C',"sol-router-$AgentId@$env:COMPUTERNAME",'-f',$KeyPath)
  if($code -ne 0 -or -not(Test-Path $PubKeyPath)){throw 'SSH key generation failed'}
}
$pub=(Get-Content $PubKeyPath -Raw).Trim()
try { Set-Clipboard -Value $pub } catch { $pub | clip.exe }
Write-Host ''
Write-Host 'ONE-TIME GITHUB STEP' -ForegroundColor Yellow
Write-Host '  1. The deploy-key page is opening now.' -ForegroundColor Yellow
Write-Host '  2. Click "Add deploy key".' -ForegroundColor Yellow
Write-Host ('  3. Title: {0}' -f $AgentId) -ForegroundColor Yellow
Write-Host '  4. Key: already copied to your clipboard; paste it.' -ForegroundColor Yellow
Write-Host '  5. IMPORTANT: check "Allow write access" and save.' -ForegroundColor Yellow
Write-Host ''
Start-Process $DeployKeyPage

$SshCommand=('ssh -i "{0}" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new -o BatchMode=yes' -f $KeyPath)
$oldGitSsh=$env:GIT_SSH_COMMAND
$env:GIT_SSH_COMMAND=$SshCommand
try {
  Step 'Waiting for the deploy key to become active (no timeout; Ctrl+C cancels)'
  $attempt=0
  while($true){
    $attempt++
    $read=Invoke-Native $git.Source @('ls-remote','--exit-code',$RepoSsh,"refs/heads/$Branch")
    if($read -eq 0){break}
    if(($attempt % 5) -eq 0){Write-Host '  still waiting for GitHub deploy key...' -ForegroundColor DarkGray}
    Start-Sleep -Seconds 2
  }

  Step 'Deploy key accepted; cloning private relay repository'
  ...
  if($clone -ne 0){throw 'SSH relay clone failed after deploy key became readable'}
  $sshConfig=('ssh -i "{0}" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new -o BatchMode=yes' -f $KeyPath)
  & $git.Source -C $RelayDir config core.sshCommand $sshConfig
  ...
  if($pushCheck -ne 0){
    Write-Host ''
    Write-Host 'The deploy key is readable but not writable.' -ForegroundColor Red
    Write-Host 'Open the deploy key settings and enable "Allow write access", then rerun this installer.' -ForegroundColor Yellow
    Start-Process $DeployKeyPage
    throw 'Deploy key does not have write access'
  }
} finally {
...
Step 'Activating Git-only transport; stopping old Cloudflare transport only now'
$self=$PID
...
Write-Host '  transport: Git polling over repository-scoped SSH deploy key'
