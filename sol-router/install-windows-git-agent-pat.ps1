param(
  [string]$Token=$env:SOL_ROUTER_GITHUB_TOKEN,
  [string]$Branch='sol-router-gateway-v0.1',
  [string]$AgentId='work-windows-cursor'
)

$ErrorActionPreference='Stop'
if([string]::IsNullOrWhiteSpace($Token)){throw 'SOL_ROUTER_GITHUB_TOKEN is empty'}
...
$RepoUrl="https://x-access-token:$EscapedToken@github.com/acaozixu123456/sol-luna-accelerator.git"
...
function Invoke-Git([string[]]$Args,[switch]$AllowFailure){
  ...
  try {
    & git.exe @Args
    $code=$LASTEXITCODE
    if(-not $AllowFailure -and $code -ne 0){throw "git failed exit=$code args=$($Args -join ' ')"}
    return $code
...
Step 'Verifying private relay access with the PAT'
$probe=Invoke-Git @('ls-remote','--exit-code',$RepoUrl,"refs/heads/$Branch") -AllowFailure
if($probe -ne 0){throw 'PAT cannot read the relay repository/branch'}

Step 'Cloning relay over HTTPS PAT'
if(Test-Path $RelayDir){Remove-Item -Recurse -Force $RelayDir}
Invoke-Git @('clone','--branch',$Branch,'--single-branch',$RepoUrl,$RelayDir)
Invoke-Git @('-C',$RelayDir,'config','user.name','sol-router-git-agent')
Invoke-Git @('-C',$RelayDir,'config','user.email','sol-router-git-agent@users.noreply.github.com')
Invoke-Git @('-C',$RelayDir,'remote','set-url','origin',$RepoUrl)
$pushProbe=Invoke-Git @('-C',$RelayDir,'push','--dry-run','origin',"HEAD:$Branch") -AllowFailure
if($pushProbe -ne 0){throw 'PAT can read but cannot write the relay repository'}
...
Write-Host '  transport: Git polling over HTTPS PAT'
