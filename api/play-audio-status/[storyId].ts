...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const upstream = await fetch(`${OLD_BACKEND}/play-audio-status/${encodeURIComponent(storyId)}`, { signal: AbortSignal.timeout(6000) });
    const body = await upstream.text();
    res.setHeader('Content-Type', upstream.headers.get('content-type') || 'application/json');
    res.setHeader('Cache-Control', 'no-store');
    return res.status(upstream.status).send(body);
  } catch (error) {
    console.error('audio status proxy failed', error);
    return res.status(502).json({ ok: false, reason: 'audio_backend_unavailable' });
  }
}
