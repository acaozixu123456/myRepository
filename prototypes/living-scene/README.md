# Cat Companion V2.1 — 动作衔接审核候选

当前端口：8777（开发）、8778（构建）。本轮增加左右起身/转向序列，反向收腿落座；去掉行走全身淡化，鼠标打断时保持当前姿态。默认趴姿与 4.5% 名义尺寸不变，展开腿尾使用留有余量的动作画布。见 [V2.1 交付](../../docs/living-scene/cat-v21/DELIVERY.md)。

# Cat Companion V2 — 当前审核候选

当前端口：8775（开发）、8776（构建）。角色参考表、三处台阶自主移动、统一姿态与验证见 [Cat V2 交付](../../docs/living-scene/cat-v2/DELIVERY.md)。名字「拿铁 / ラテ」已获用户确认。尺寸保持 4.5%；安静停留约一分钟可见偶尔移动，靠近/抚摸会延后出发。下方保留历史版本说明。
