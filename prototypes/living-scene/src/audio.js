...
class LivingSceneAudio {
  ...
  setHover(value) {
    ...
    this.hovered = value;
    this._applySceneState();
    if (value && this.enabled && performance.now() - this.lastHoverCue > 900) {
      this.lastHoverCue = performance.now();
      this._bell(659.25, 0, 0.019, this.pointerX * 0.35);
    }
  }
  ...
  _applySceneState() {
    if (!this.ctx) return;
    const now = this.ctx.currentTime;
    const shop = this.focused ? 0.19 : this.hovered ? 0.085 : 0.0001;
    const music = this.focused ? 0.074 : 0.105;
    const ambience = this.focused ? 0.062 : 0.085;
