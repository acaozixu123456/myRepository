...
const out=path.join(root,'docs/living-scene/cat-v2/evidence');
const capture='/Users/xiaruonan/nihongo-art-work/toolchain/cat-v2-capture';
const checks={},events=[];
...
try {
 await p.goto('http://127.0.0.1:8776/?qa=1');await p.bringToFront();await p.waitForFunction(()=>window.__livingScene?.ready);
 ...
 const recording=await recordBrowser(p,capture,async page=>{
   ...
   for(let i=0;i<8;i++){await page.waitForTimeout(400);events.push((await snap()).cat);if(i===3)await shot('mid-walk');}
   ...
   check('walkingFramesVisible',events.some(e=>e.travelMix>.9));
   ...
   await page.mouse.move(800,420);
   await page.waitForFunction(()=>{const d=window.__livingScene.snapshot().cat.director;return d.nextMove-d.clock<2;},{},{timeout:90000});
 }});
