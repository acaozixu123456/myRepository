...
export function createWorld(canvas:HTMLCanvasElement,initial:Progress,events:{near:(t:Target|null)=>void;interact:(id:TargetId)=>void;lock:(v:boolean)=>void;lost:()=>void}):World{
  ...
  const roadMat=mat('#62635f');const roadTex=new DynamicTexture('paving',{width:512,height:512},scene,false);const c=roadTex.getContext();
  ...
  for(let i=0;i<ROAD.length-1;i++){
    ...
    const paving=box(7.3,.1,len+.18,0,0,0,'#62635f',root,true);paving.material=roadMat;
