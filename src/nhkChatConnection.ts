...
export type ChatHooks={phase:(p:ChatPhase)=>void;assistant:(s:string)=>void;hint:(s:string)=>void;blocked:(v:boolean)=>void;error:(reason:string,retryAfter?:number)=>void;shuffle:()=>void;heard:(text:string)=>void};
type Ticket={callId:string;expiresAt:number;stopToken:string};
type Job={kind:ChatAction;heard:string};
...
export class NhkChatConnection{
  ...
  constructor(private plan:ChatPlan,private hooks:ChatHooks){}
  async start(){if(this.closed||this.stream||this.busyConnecting)return;this.busyConnecting=true;this.hooks.phase('connecting');try{
    ...
    this.stream=stream;this.mic(false);this.busyConnecting=false;await this.connect({kind:'start',heard:''},false);
  }catch(e){this.busyConnecting=false;if(!this.closed)this.fail(e instanceof Error?`${e.name}:${e.message}`:'connection_failed');}}
  ...
  private async connect(job:Job,renew:boolean){
    ...
    try{
      const pc=this.pc=new RTCPeerConnection();const audio=this.audio=document.createElement('audio');audio.autoplay=true;audio.setAttribute('playsinline','');audio.style.display='none';document.body.appendChild(audio);
      pc.ontrack=e=>{if(this.closed||serial!==this.serial)return;audio.srcObject=e.streams[0]||new MediaStream([e.track]);void audio.play().catch(()=>{if(!this.closed&&serial===this.serial){this.blocked=true;this.mic(false);this.hooks.blocked(true);}});};
      ...
      this.stream.getTracks().forEach(t=>pc.addTrack(t,this.stream!));const dc=this.dc=pc.createDataChannel('oai-events');let negotiated=false;
  ...
  private rotate(job:Job){if(this.closed||this.busyConnecting)return;clearTimeout(this.idle);this.pending=null;void this.connect(job,true);}
  private request(job:Job){
    if(this.closed)return;this.mic(false);this.waiting=false;this.talking=false;clearTimeout(this.idle);
    ...
    this.hooks.assistant('');this.hooks.phase('coach');
    // Detached responses prevent earlier topics/audio from leaking into a new topic and bound context cost.
    this.emit({type:'response.create',response:{conversation:'none',input:[],output_modalities:['audio'],instructions:chatInstructions(this.plan,job.kind,this.history,job.heard),metadata:{purpose:CHAT_CONTRACT,seq:String(this.requestSeq),topic:String(this.topicSerial)}}});
    clearTimeout(this.watchdog);this.watchdog=setTimeout(()=>this.fail('response_timeout'),28000);
  }
  changeTopic(plan:ChatPlan){if(this.closed)return;this.plan=plan;this.topicSerial++;this.history=[];this.committed.clear();this.mic(false);this.waiting=false;this.talking=false;this.output='';this.hooks.assistant('');this.hooks.hint('换个轻松的话头。一个词也可以。');clearTimeout(this.idle);clearTimeout(this.shuffleTimer);
  ...
  help(){this.request({kind:'help',heard:''});}repeat(){this.request({kind:'repeat',heard:''});}
  ...
  private event(e:any){if(!e||typeof e.type!=='string')return;
    ...
    switch(e.type){
      ...
      case'response.output_audio_transcript.done':if(!this.pending&&!this.shuffleTimer){this.output=String(e.transcript||this.output).slice(0,1000);this.hooks.assistant(this.output);}break;
      case'output_audio_buffer.started':this.playbackDone=false;this.mic(false);break;
      ...
      case'response.done':{
        ...
        this.responseDone=true;this.release();break;
      }
      case'input_audio_buffer.speech_started':if(this.accepting){this.talking=true;this.waiting=false;clearTimeout(this.idle);this.hooks.phase('listening');this.hooks.hint('我在听，慢慢说。');}break;
      ...
      case'conversation.item.input_audio_transcription.completed':{
        ...
        if(intent==='help'||intent==='repeat'){this.request({kind:intent,heard:text});break;}
        this.hooks.heard(text);this.addHistory('user',text);this.request({kind:'answer',heard:text});break;
      }
  ...
  dispose(){if(this.closed)return;this.closed=true;clearTimeout(this.idle);clearTimeout(this.shuffleTimer);this.closePeer();this.stream?.getTracks().forEach(t=>t.stop());this.stream=null;this.history=[];}
