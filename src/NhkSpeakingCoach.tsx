...
export default function NhkSpeakingCoach({article,preferredSentence=''}:Props){
  ...
  useEffect(()=>{const pause=()=>{if(connection.current){void connection.current.setMicEnabled(false);connection.current.end();setHint('已暂停新闻陪练，先专注讲解；需要时可以重新开始。');}};document.addEventListener('hitokoto-study-focus',pause);return()=>document.removeEventListener('hitokoto-study-focus',pause);},[]);
