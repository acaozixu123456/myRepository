...
describe('manual learning support takes priority over stale observations',()=>{
 ...
 it('seeing an unrelated note does not mark the next turn assisted or grant mastery',()=>{const {c,impl}=setup();const before=impl.policy.independent;c.exposeNote('n');expect(impl.pendingAssistance).toBe('none');expect(impl.policy.independent).toBe(before);const epoch=impl.policyEpoch;c.exposeNote('n');expect(impl.policyEpoch).toBe(epoch);c.dispose();});
