...
export function noteStillApplies(note:WrittenNote,lines:Line[],automatic=true):boolean{
 if(!automatic&&note.mode==='auto')return false;const index=lines.findIndex(l=>l.id===note.anchorId);const line=lines[index];
 ...
 if(note.mode!=='help'&&line.role==='user')for(const next of lines.slice(index+1)){
  ...
  if(next.role==='user')return false; // A continued fragment or self-repair supersedes the earlier fragment.
 }
 return true;
}
...
export class WrittenLane {
 ...
 update(lines:Line[],target:number){
  if(this.stopped)return;this.lines=lines;this.target=target;const r=this.automatic();
  const fingerprint=r?JSON.stringify([r.anchorId,r.source,r.context.map(l=>[l.id,l.text])]):'';
 ...
 setSpeaking(value:boolean){this.speaking=value;if(value){clearTimeout(this.timer);}else{this.deliver();const r=this.automatic();if(r&&this.fingerprint!==this.lastAttempt&&(this.enabled||r.mode==='question')&&!this.abort&&!this.waiting)this.schedule(r,900);}}
 setReading(value:boolean){this.reading=value;if(!value)this.deliver();}
 setEnabled(value:boolean){this.enabled=value;if(!value)this.invalidate();else{this.fingerprint='';this.lastAttempt='';this.update(this.lines,this.target);}}
