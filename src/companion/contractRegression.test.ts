...
describe('observed native preview regressions',()=>{
 ...
 it('requires current concise guidance and leaves native audio intact',()=>{const s=companionInstructions(LOCAL_SEEDS[0],freshPolicy());expect(s).toContain('一个简短回应，或者一个容易接的问题');expect(s).toContain('Keep unsolicited teaching silent');expect(s).toContain('explicitly asked language questions are NOT unsolicited');expect(s).toContain('permanent low ceiling');const connection=readFileSync('src/companion/connection.ts','utf8');expect(connection).not.toContain('TeacherTurnChannel');expect(connection).not.toContain("conversation:'none'");});
