...
export type Hooks={phase:(p:Phase)=>void;lines:(l:Line[])=>void;activity:(v:VoiceActivity)=>void;notice:(s:string)=>void;error:(s:string)=>void;policy:(p:Policy)=>void};
...
export class CompanionConnection {
  ...
  private responseId='';private responseSeq=0;private activeSeq=0;private currentAssistant='';private lastAssistant='';private playbackEnded=true;private generationEnded=true;private discarded=new Set<string>();private topicEpoch=0;private hasVoice=false;
  ...
  private changed(){this.hooks.lines(this.ledger.ordered());}
  async start(){
    ...
    try{
      ...
      pc.onconnectionstatechange=()=>{if(!this.ended&&['disconnected','failed','closed'].includes(pc.connectionState))this.fail('connection_lost');};
  ...
  private async keepAlive(){if(this.ended||!this.ticket||this.heartbeatBusy)return;this.heartbeatBusy=true;try{await companionApi('heartbeat',{...this.ticket},this.abort.signal);}catch(e){if(!this.ended)this.fail(e instanceof Error?e.message:'heartbeat_failed');}finally{this.heartbeatBusy=false;}}
  async setMic(on:boolean){
    if(this.ended||!this.linked||on===this.wantsMic)return;this.wantsMic=on;const request=++this.micRequest;
    if(!on){const unfinished=this.gate.speaking;this.gate.speaking=false;this.acquiring=false;this.stream?.getTracks().forEach(t=>t.stop());this.stream=null;this.meter.detach('input');void this.sender?.replaceTrack(null).catch(()=>{});if(unfinished)this.emit({type:'input_audio_buffer.commit'});this.publishActivity();return;}
  ...
  private interrupt(){
    ...
    if(this.currentAssistant){this.ledger.upsert(this.currentAssistant,'assistant',{interrupted:true,delivered:false});this.changed();}
  ...
  private async observe(){
    ...
    this.observing=true;this.observedAt=this.userTurns;const epoch=this.topicEpoch,revision=this.policy.revision;const abort=this.observerAbort=new AbortController();
    try{const observations=await fetchObservations(this.ticket,evidence,AbortSignal.any([abort.signal,this.abort.signal]));if(this.ended||epoch!==this.topicEpoch||revision!==this.policy.revision)return;const next=applyObservations(this.policy,observations,evidence);if(this.gate.active||this.gate.speaking)this.nextPolicy=next;else{this.policy=next;this.hooks.policy(next);this.configure();}}
  ...
  private event(e:any){
    ...
    switch(e.type){
      ...
      case'input_audio_buffer.speech_started':if(this.wantsMic&&this.stream){this.gate.speaking=true;this.queuedAction=null;clearTimeout(this.idle);this.interrupt();this.hooks.phase('ready');this.publishActivity();}break;
      case'input_audio_buffer.speech_stopped':this.gate.speaking=false;if(this.gate.hasPending)this.flushSoon();break;
      case'input_audio_buffer.committed':{const id=String(e.item_id||'');if(!this.gate.commit(id))break;this.gate.speaking=false;this.ledger.upsert(id,'user',{previous:String(e.previous_item_id||''),assistance:this.pendingAssistance});this.pendingAssistance='none';this.userTurns++;this.hasVoice=true;this.flushSoon();break;}
