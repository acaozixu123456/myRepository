...
export class CompanionConnection {
  ...
  private queuedAction:Action|null=null;private speed=1;private pace:SpeakingPace='natural';private teaching:TeachingChannel='automatic';private lastInput:'speech'|'typed'='speech';private textResponse=false;private practiceMode=false;private studyPaused=false;private notesEnabled=true;private demoLoading=false;private demoEpoch=0;private demoAbort:AbortController|null=null;private replay:RemoteReplay;private supportExamples:Array<{text:string;until:number}>=[];private practiceContext='';private captionWaitStarted=0;private nextPolicy:Policy|null=null;private pendingAssistance:Line['assistance']='none';private userTurns=0;private observedAt=0;private observing=false;
  ...
  private changed(){const lines=this.ledger.ordered();this.hooks.lines(lines);if(!this.practiceMode&&!this.studyPaused)this.written?.update(lines.filter(l=>!this.earlierTopicItems.has(l.id)),this.policy.target);}
  ...
  setTopic(seed:Seed,deferOpening=false){if(this.ended)return;this.practiceMode=false;this.practiceContext='';this.supportExamples=[];this.replay.clear();this.seed=seed;this.topicEpoch++;this.earlierTopicItems=new Set(this.ledger.ordered().map(l=>l.id));this.written?.reset();this.observerAbort?.abort();this.nextPolicy=null;this.interrupt();this.gate.clear();if(!deferOpening)this.action('topic');}
  async pauseForStudy(){if(this.ended)return;this.studyPaused=true;await this.setMic(false);this.interrupt();this.queuedAction=null;this.gate.clear();this.written?.setReading(true);this.hooks.phase('ready');this.hooks.notice('专注学习 · 麦克风已暂停，返回后手动开麦。');}
  resumeAfterStudy(){this.studyPaused=false;this.written?.setReading(false);this.hooks.notice('准备好了，再点麦克风继续。');}
  ...
  private flush(){
    if(this.ended||!this.linked||this.gate.speaking||this.gate.active||this.practiceMode||this.studyPaused)return;
