...
export default function CompanionApp(){
  ...
  const replenish=async(wanted:Lane,selectWhenReady=false)=>{
    ...
    catch(e){if(!abort.signal.aborted&&seq===topicSeq.current)setTopicNote(wanted==='news'?'这次消息来源或内容校验没接好，原话题仍可聊。':'新话头稍后再来，现有的话题照样能聊。');}finally{if(seq===topicSeq.current){topicBusy.current=false;setBusyTopics(false);}}
  };
    ...
    <div className="kc-shell">
      ...
      {view==='chat'&&<>
        <main className="kc-conversation" ref={scroller} aria-label="当前对话" onScroll={e=>{const el=e.currentTarget;followBottom.current=el.scrollHeight-el.scrollTop-el.clientHeight<70;}}>
          ...
          {showText&&visible.map((line,index)=><article key={line.id} data-line-id={line.id} className={`kc-line ${line.role} ${line.id===lastId?'latest':''} ${index<visible.length-2?'earlier':''}`}><span className="kc-line-label">{line.role==='assistant'?'ひとこと':'你'}</span><p lang={line.role==='assistant'?'ja':undefined}>{line.text}</p>{line.interrupted&&<small>刚才这一句已打断</small>}{!line.interrupted&&notes.filter(n=>n.anchorId===line.id&&noteStillApplies(n,lines,writtenEnabled)).map(n=><WrittenNoteCard key={n.id} note={n} expanded={expandedNote===n.id} onToggle={()=>toggleNote(n.id)} onDismiss={()=>{conn.current?.dismissNote(n.anchorId);setNotes(old=>old.filter(x=>x.id!==n.id));readingNote.current=false;conn.current?.setReadingNote(false);}} onReading={value=>{readingNote.current=value;if(value)followBottom.current=false;conn.current?.setReadingNote(value);}} onSeen={()=>conn.current?.exposeNote(n.id)}/>)}</article>)}
