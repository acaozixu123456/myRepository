...
export default function CompanionApp(){
  ...
  const finish=()=>{setPractice(null);setSheet(null);conn.current?.end();setLines([]);setNotes([]);setNotePending(false);setNoteError('');setUnreadNote('');};
  ...
  const startPractice=(subject:Subject,previousScene='')=>{setScenery(false);if(view!=='chat'||!conn.current||['closed','error','connecting'].includes(phase)){setTextPractice(subject);setSheet('standalone-practice');return;}setSheet(null);setPractice({key:crypto.randomUUID(),subject,previousScene});followBottom.current=true;};
  ...
  return <div className={`kc-root ${wallpaper?'imm-wallpaper':''}`} data-companion="native-v3" data-release="repair-20260911" data-ui-release="neon-20260911" data-entry-release="entry-20260911" data-teacher-release="teacher-20260911-v2" data-immersion-release="immersion-20260911-v3" data-scenery={scenery&&!practice} data-view={view} data-motion={motionOn?'full':'reduced'} data-signal={voiceVisual(phase,activity).key}>
    <AmbientStage key={ambientKey} quality={quality} motion={motionOn} paused={studyOpen||!!sheet} onStatus={setAmbientStatus}/>
    ...
    <div className="kc-shell">
      <header className="kc-header">
        ...
        {view==='chat'?<div className="kc-chat-heading"><span>日语，慢慢聊</span><button onClick={()=>setSheet('topics')}>{seed.title}<ChevronDown size={13}/></button></div>:<span className="imm-edition">IMMERSION / 03</span>}
      ...
      {view==='chat'&&<>
        <div className="imm-view-toggle"><button aria-pressed={scenery} onClick={()=>setScenery(true)}>风景陪练</button><button aria-pressed={!scenery} onClick={()=>setScenery(false)}>对话与历史</button><button onClick={()=>setSheet('scene')}><SceneIcon size={12}/> 风景</button></div>
        {scenery&&!practice&&<div className="imm-scene-caption">RAIN / MIDNIGHT<small>此刻，只说你想说的一句。</small></div>}
    ...
    <dialog className="kc-sheet" ref={dialog} onCancel={()=>setSheet(null)} onClick={e=>{if(e.target===e.currentTarget)setSheet(null);}} aria-labelledby="kc-sheet-title"><div className="kc-sheet-inner"><div className="kc-sheet-handle"/><header><h2 id="kc-sheet-title">{sheet==='topics'?'今天，想往哪儿聊？':sheet==='write'?'先用文字说也可以':sheet==='sources'?'这则消息的出处':sheet==='expressions'?'我的表达':sheet==='segments'?'分句听 · 重新示范':sheet==='save-consent'?'保存到本机？':sheet==='custom'?'我来定主题':sheet==='scene'?'雨夜 · 新东京':sheet==='standalone-practice'?'先练一句 · 文字练习':'按你的节奏来'}</h2><button className="kc-icon" aria-label="关闭面板" onClick={()=>setSheet(null)}><X size={21}/></button></header>
      ...
      {sheet==='settings'&&<>
        <div className="neon-settings-brand"><img src="/art/hitokoto-partner.webp" width="48" height="48" alt=""/><span><strong>HITOKOTO</strong><small>你的日语聊天伙伴</small></span><b>IMMERSION / 03</b></div>
        ...
        </div><p className="kc-sheet-footnote">可以随时问词义、语法和怎么说，不再要求特殊口令。短练习可随时跳过。版本 9.11 · IMMERSION 03 / TEACHER 02。实时语音：gpt-realtime-2.1 / marin；文字教师：gpt-5.4-mini。AI 原音仅在当前会话内存中用于重播，结束即释放，不录制你的声音。<br/>语音由 AI 生成；开启麦克风后，声音会传给 OpenAI。</p>
      </>}
      {sheet==='custom'&&<CustomTopicSheet brief={brief} onChange={setBrief} onPrepared={customReady} onCancelPreparation={cancelRandom}/>}
      {sheet==='scene'&&<section className="imm-scene-settings"><small>IMMERSION 03 · AMBIENT STUDIO</small><h3>雨夜 · 新东京</h3><p>城市灯海、窗边雨痕与夜行车流。风景静音，与你的语音完全分开。</p><div className="imm-quality">{([['auto','自动 · 推荐'],['1080','流畅 · 1080'],['1440','高画质 · 1440'],['2160','超清 · 2160']] as const).map(([q,label])=><button key={q} aria-pressed={quality===q} onClick={()=>setQuality(q)}>{label}</button>)}</div><p className="imm-status">{ambientStatus||'画面就绪后开始播放；优先保持聊天流畅。'}</p><button className="imm-custom-entry" onClick={()=>setMotionOn(v=>!v)}>{motionOn?'减少动态，保留高清静帧':'启用动态风景与粒子'}</button><button className="imm-custom-entry" onClick={()=>{setAmbientStatus('');setAmbientKey(v=>v+1);}}>重新加载风景</button><button className="imm-primary" onClick={()=>{void conn.current?.pauseForStudy();setWallpaper(true);setSheet(null);}}><Maximize2 size={18}/>看全景 · 暂停麦克风</button><p><small>原创 AI 环境母图＋分层动画，横竖独立构图；静音循环，不在聊天时实时渲染整座城市。超清会增加下载与解码负担，掉帧时自动降低画质。</small></p></section>}
