...
type Sheet='topics'|'settings'|'write'|'sources'|'expressions'|'segments'|'save-consent'|null;
...
export default function CompanionApp(){
  ...
  const begin=()=>{conn.current?.dispose();const id=++generation.current;setView('chat');setPhase('connecting');setPractice(null);setSessionIds([]);lastPracticeTurn.current=0;setReplayReady(false);setLines([]);setNotes([]);setExpandedNote('');setNotePending(false);setNoteError('');setUnreadNote('');lastLearner.current='';readingNote.current=false;followBottom.current=true;setActivity(silent);setError('');setNotice('');
    const current=()=>generation.current===id;const c=new CompanionConnection(seed,policy.current,{phase:p=>{if(current()){setPhase(p);if(p==='closed'){setPractice(null);setView('end');}}},lines:l=>{if(current()){const latest=[...l].reverse().find(v=>v.role==='user')?.id||'';if(latest&&latest!==lastLearner.current){lastLearner.current=latest;readingNote.current=false;followBottom.current=true;}setLines(l);}},activity:a=>{if(current())setActivity(a);},notice:s=>{if(current())setNotice(s);},error:s=>{if(current())setError(friendlyError(s));},written:n=>{if(current()){setNotes(old=>[...old.filter(x=>x.anchorId!==n.anchorId),n].slice(-24));setUnreadNote(n.id);if(!readingNote.current)setExpandedNote(n.id);}},replay:ready=>{if(current())setReplayReady(ready);},writtenPending:v=>{if(current())setNotePending(v);},writtenError:s=>{if(current())setNoteError(s);},policy:p=>{if(current()){policy.current=p;if(memoryRef.current)try{writeLearning(localStorage,p);}catch{setNotice('这次的练习节奏没有保存，聊天不受影响。');}}}});
    conn.current=c;c.setPace(speed);c.setWrittenEnabled(writtenEnabled);c.setTeachingChannel(teachingChannel);void c.start(!!queuedReview.current);
  };
  ...
  const startPractice=(subject:Subject,previousScene='')=>{setSheet(null);setPractice({key:crypto.randomUUID(),subject,previousScene});followBottom.current=true;};
  ...
  return <div className="kc-root" data-companion="native-v3" data-release="repair-20260911" data-ui-release="neon-20260911" data-entry-release="entry-20260911" data-teacher-release="teacher-20260911-v2" data-view={view} data-motion={motionOn?'full':'reduced'} data-signal={voiceVisual(phase,activity).key}>
    <div className="kc-shell">
      ...
      {view==='chat'&&<>
        ...
        <main className="kc-conversation" ref={scroller} aria-label="当前对话" onScroll={e=>{const el=e.currentTarget;followBottom.current=el.scrollHeight-el.scrollTop-el.clientHeight<70;}}>
          ...
          {visible.filter(line=>showText||line.textOnly).map((line,index)=><article key={line.id} data-line-id={line.id} className={`kc-line ${line.role} ${line.id===lastId?'latest':''} ${index<visible.length-2?'earlier':''}`}><span className="kc-line-label">{line.role==='assistant'?'ひとこと / AI':line.exercise?'你 / 练习':'你 / YOU'}</span><p>{line.text}</p>{line.interrupted&&<small>刚才这一句已打断</small>}{line.role==='assistant'&&line.delivered&&!line.interrupted&&<button className="teacher-explain-line" disabled={!!practice} onClick={()=>conn.current?.sendText(`请用中文解释这句日语的意思和一个关键用法：「${line.text.slice(0,420)}」`)}>解释这句</button>}{!line.interrupted&&eligibleNotes.filter(n=>n.anchorId===line.id).map(noteCard)}</article>)}
    ...
    <dialog className="kc-sheet" ref={dialog} onCancel={()=>setSheet(null)} onClick={e=>{if(e.target===e.currentTarget)setSheet(null);}} aria-labelledby="kc-sheet-title"><div className="kc-sheet-inner"><div className="kc-sheet-handle"/><header><h2 id="kc-sheet-title">{sheet==='topics'?'今天，想往哪儿聊？':sheet==='write'?'先用文字说也可以':sheet==='sources'?'这则消息的出处':sheet==='expressions'?'我的表达':sheet==='segments'?'分句听 · 重新示范':sheet==='save-consent'?'保存到本机？':'按你的节奏来'}</h2><button className="kc-icon" aria-label="关闭面板" onClick={()=>setSheet(null)}><X size={21}/></button></header>
      ...
      {sheet==='settings'&&<>
        <div className="neon-settings-brand"><img src="/art/hitokoto-partner.webp" width="48" height="48" alt=""/><span><strong>HITOKOTO</strong><small>你的日语聊天伙伴</small></span><b>TEACHER / 02</b></div>
        ...
        </div><p className="kc-sheet-footnote">可以随时问词义、语法和怎么说，不再要求特殊口令。短练习可随时跳过。版本 9.11 · TEACHER 02。实时语音：gpt-realtime-2.1 / marin；文字教师：gpt-5.4-mini。AI 原音仅在当前会话内存中用于重播，结束即释放，不录制你的声音。<br/>语音由 AI 生成；开启麦克风后，声音会传给 OpenAI。</p>
      </>}
