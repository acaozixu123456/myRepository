...
export default function CompanionApp(){
  ...
  const replenish=async(wanted:Lane,selectWhenReady=false)=>{
    ...
    catch(e){if(!abort.signal.aborted&&seq===topicSeq.current)setTopicNote(wanted==='news'?'暂时没找到可靠的新消息，先聊别的也好。':'新话头稍后再来，现有的话题照样能聊。');}finally{if(seq===topicSeq.current){topicBusy.current=false;setBusyTopics(false);}}
  };
