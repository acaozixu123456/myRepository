...
export default function CompanionApp(){
  ...
  return <div className={`kc-root ${wallpaper?'imm-wallpaper':''}`} data-companion="native-v3" data-release="repair-20260911" data-ui-release="neon-20260911" data-entry-release="entry-20260911" data-teacher-release="teacher-20260911-v2" data-immersion-release="immersion-20260911-v3" data-desktop-release="desktop-20260911-v4" data-desktop-focus={desktop} data-practice={!!practice} data-scenery={scenery&&!practice} data-view={view} data-motion={motionOn?'full':'reduced'} data-signal={voiceVisual(phase,activity).key}>
    ...
    <div className="kc-shell">
      <header className="kc-header">
        ...
        {view==='chat'?<div className="kc-chat-heading"><span>日语，慢慢聊</span><button onClick={()=>setSheet('topics')}>{seed.title}<ChevronDown size={13}/></button></div>:<span className="imm-edition">{desktop?'DESKTOP / 04':'IMMERSION / 03'}</span>}
    ...
    <dialog className="kc-sheet" ref={dialog} onCancel={()=>setSheet(null)} onClick={e=>{if(e.target===e.currentTarget)setSheet(null);}} aria-labelledby="kc-sheet-title"><div className="kc-sheet-inner"><div className="kc-sheet-handle"/><header><h2 id="kc-sheet-title">{sheet==='topics'?'今天，想往哪儿聊？':sheet==='write'?'先用文字说也可以':sheet==='sources'?'这则消息的出处':sheet==='expressions'?'我的表达':sheet==='segments'?'分句听 · 重新示范':sheet==='save-consent'?'保存到本机？':sheet==='custom'?'我来定主题':sheet==='scene'?'雨夜 · 新东京':sheet==='standalone-practice'?'先练一句 · 文字练习':'按你的节奏来'}</h2><button className="kc-icon" aria-label="关闭面板" onClick={()=>setSheet(null)}><X size={21}/></button></header>
      ...
      {sheet==='settings'&&<>
        <div className="neon-settings-brand"><img src="/art/hitokoto-partner.webp" width="48" height="48" alt=""/><span><strong>HITOKOTO</strong><small>你的日语聊天伙伴</small></span><b>DESKTOP / 04</b></div>
