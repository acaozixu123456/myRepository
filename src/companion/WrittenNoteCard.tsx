...
type Props={note:WrittenNote;expanded:boolean;onToggle:()=>void;onDismiss:()=>void;onSeen:()=>void;onReading?:(value:boolean)=>void};
/** No live-region announcement: a quiet visual aid, not another voice. */
export function WrittenNoteCard({note,expanded,onToggle,onDismiss,onSeen,onReading}:Props){
 ...
 return <aside ref={card} className="kc-written-note" data-note-for={note.anchorId} data-note-kind={note.kind} aria-label="安静的文字提示">
  ...
  {expanded&&<div className="kc-note-body" onPointerDown={()=>onReading?.(true)}>
   ...
   <details className="kc-note-detail" onToggle={e=>onReading?.(e.currentTarget.open)}><summary>看一小点说明</summary>{note.detailZh&&<p lang="zh-CN">{note.detailZh}</p>}<small>根据刚才的字幕和语境提供参考，不是发音评分。</small><button onClick={onDismiss}>识别不对，先收起</button></details>
  </div>}
 </aside>;
}
